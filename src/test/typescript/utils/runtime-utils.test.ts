...
describe('Runtime Utils', () => {
  ...
  describe('assertParamExists', () => {
    it('should not throw when parameter exists', () => {
      expect(() =>
        assertParamExists('testFunction', 'testParam', 'value')
      ).not.toThrow();
      expect(() =>
        assertParamExists('testFunction', 'testParam', 0)
      ).not.toThrow();
      expect(() =>
        assertParamExists('testFunction', 'testParam', false)
      ).not.toThrow();
    });

    it('should throw RequiredError when parameter is null', () => {
      expect(() =>
        assertParamExists('testFunction', 'testParam', null)
      ).toThrow(RequiredError);
    });

    it('should throw RequiredError when parameter is undefined', () => {
      expect(() =>
        assertParamExists('testFunction', 'testParam', undefined)
      ).toThrow(RequiredError);
    });
